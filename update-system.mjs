...
export function localUserPaths(root = ROOT) {
  ...
  for (const path of declared) {
    ...
    // Warned, not refused. The refusal this replaces assumed a SYSTEM_PATHS
    // match proves upstream ships the path, and it does not:
    //
    //   - a directory entry is a wildcard, so `providers/` covers upstream's
    //     greenhouse.mjs and a provider that exists only in this fork alike;
    //   - even an exact entry can be stale, naming a file the fetched tree no
    //     longer has (the checkout loop below has always tolerated exactly
    //     that, calling it "a path genuinely absent upstream").
    //
    ...
    // undeclarable, and the stale-file prune then deleted them as though
    // upstream had dropped them. Answering "does upstream ship this?" needs the
    // fetched tree, which this function cannot reach: it is also called by the
    // offline coverage validator. Warning keeps the original rationale intact
    // where it matters, since that rationale was about *silence*, not refusal:
    // fork a genuinely shipped file and you are told it stops updating.
...
async function apply() {
  ...
  try {
    ...
    // Drives only the "would the exclusions cancel this whole pathspec?" guard
    // below, so it tracks preserveSpecs, not the .bak messaging (which stays
    // keyed to atRisk — a declared path is not necessarily a modified one).
    ...
    for (const path of updatePaths) {
      ...
      if (preservedSet.size > 0) {
        const preservedHere = keptFromCheckout.filter((f) => (path.endsWith('/') ? f.startsWith(path) : f === path));
