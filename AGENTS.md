...
| `apply-route.mjs` | Singapore apply-route check — is a posting's Apply button a MyCareersFuture handoff (Singpass-gated, so unapplicable without an NRIC/FIN)? Exact for foundit (its API returns the destination `applyUrl`); INFERRED for JobStreet by asking MCF whether the advertiser posts there, cached per company in `data/.mcf-company-cache.json`. Annotates `applyRoute` (`gated` / `likely-gated` / `open` / `unknown`), never drops a posting, and renders as the digest's Route column. A `likely-gated` flag means verify the apply button before writing a tailored CV — not "skip it" |
